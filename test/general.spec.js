...
describe("BYE handling", function () {
    it("should propagate BYEs through the brackets", async function () {
        ...
        assert.strictEqual(
            (await this.storage.select("match", 2)).opponent1.id,
            0
        );
        assert.strictEqual(
            (await this.storage.select("match", 2)).opponent2,
            null
        );

        assert.strictEqual(
            (await this.storage.select("match", 3)).opponent1,
            null
        );
        assert.strictEqual(
            (await this.storage.select("match", 3)).opponent2,
            null
        );

        assert.strictEqual(
            (await this.storage.select("match", 4)).opponent1,
            null
        );
        assert.strictEqual(
            (await this.storage.select("match", 4)).opponent2,
            null
        );

        assert.strictEqual(
            (await this.storage.select("match", 5)).opponent1.id,
            0
        );
        assert.strictEqual(
            (await this.storage.select("match", 5)).opponent2,
            null
        );
    });
    ...
    it("should handle incomplete seeding during creation", async function () {
        ...
        assert.strictEqual(
            (await this.storage.select("match", 0)).opponent1.id,
            0
        );
        assert.strictEqual(
            (await this.storage.select("match", 0)).opponent2.id,
            1
        );

        assert.strictEqual(
            (await this.storage.select("match", 1)).opponent1,
            null
        );
        assert.strictEqual(
            (await this.storage.select("match", 1)).opponent2,
            null
        );
    });
    ...
    it("should balance BYEs in the seeding", async function () {
        ...
        assert.strictEqual(
            (await this.storage.select("match", 0)).opponent2,
            null
        );

        assert.strictEqual(
            (await this.storage.select("match", 1)).opponent1.id,
            1
        );
        assert.strictEqual(
            (await this.storage.select("match", 1)).opponent2,
...
describe("Update match child count", function () {
    ...
    it("should change match child count at match level", async function () {
        ...
        assert.strictEqual(
            (await this.storage.select("match", 0)).child_count,
            3
        );
        assert.strictEqual(
            (await this.storage.select("match_game")).length,
            6 + 3
        );
    });
...
describe("Seeding and ordering in elimination", function () {
    ...
    it("should throw if at least one match is running or completed", async function () {
        ...
        await assert.isRejected(
            this.manager.update.roundOrdering(0, "natural"),
    ...
    it("should update all the ordering of a stage at once", async function () {
        await this.manager.update.ordering(0, [
            "pair_flip",
            "half_shift",
            "reverse",
            "natural",
...
describe("Best-Of series matches completion", function () {
    ...
    it("should handle match auto-win against a BYE after a BoX series", async function () {
        ...
        assert.strictEqual(
            (await this.storage.select("match", 4)).opponent1.result,
            "win"
        );
        assert.strictEqual(
            (await this.storage.select("match", 6)).opponent1.result,
            "win"
        );
    });
});
...
describe("Reset match and match games", function () {
    ...
    it("should throw when at least one of the following match is locked", async function () {
        ...
        await assert.isRejected(
            this.manager.reset.matchResults(0),
            "The match is locked."
        );
    });
    ...
    it("should reset results of a match game", async function () {
        ...
        assert.strictEqual(
            (await this.storage.select("match", 7)).opponent1,
            null
        ); // BYE in consolation final.
        ...
        assert.strictEqual(
            (await this.storage.select("match", 7)).opponent1,
            null
        ); // Still BYE in consolation final.
    });
});
...
describe("Import / export", function () {
    it("should import data in the this.storage", async function () {
        ...
        assert.strictEqual(
            (await this.storage.select("match", 0)).opponent1.result,
            "win"
        );
        assert.strictEqual(
            (await this.storage.select("match", 1)).opponent1.result,
            "win"
        );
    ...
    it("should import data in the this.storage with normalized IDs", async function () {
        ...
        assert.deepEqual(initialData.participant[0], {
            id: 1,
            tournament_id: tournamentId,
            name: "Team 1",
        });
        assert.deepEqual(initialData.group[0], {
            id: 1,
            stage_id: stage2.id,
            number: 1,
        });
        assert.deepEqual(initialData.round[0], {
            id: 3,
            stage_id: stage2.id,
            group_id: 1,
            number: 1,
        ...
        assert.deepEqual(data.participant[0], {
            id: 0,
            tournament_id: tournamentId,
            name: "Team 1",
        });
        assert.deepEqual(data.group[0], {
            id: 0,
            stage_id: stage1.id,
            number: 1,
        });
        assert.deepEqual(data.round[0], {
            id: 0,
            stage_id: stage1.id,
            group_id: 0,
            number: 1,
    ...
    it("should export data from the this.storage", async function () {
        ...
        assert.deepEqual(
            await this.storage.select("participant"),
            data.participant
        );
        ...
        assert.deepEqual(
            await this.storage.select("match_game"),
            data.match_game
        );
    });
});
